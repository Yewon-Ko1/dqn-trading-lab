...
초기 실험에서 에이전트는 테스트 연도의 첫 관측창만큼 거래를 건너뛰었지만 Buy & Hold는 연초부터 평가되는 오류를 발견했습니다. 관측창 길이에 따라 유·불리가 달라져 기존 window 비교와 2021~2024 결과를 최종 근거에서 제외했습니다.
...
기술지표, 장기 추세, 시장지수, 투자자 수급, 변동성 국면, 미국 지수 등 여러 피처 구성을 비교했지만 복잡한 입력이 안정적인 개선으로 이어지지는 않았습니다. 반면 잦은 매매를 제한하는 실험은 거래 횟수와 거래비용을 크게 낮췄습니다.

이 결과를 바탕으로 현재는 작은 피처 세트와 거래 제약을 기준으로 삼고, 보상 패널티·국면 조건부 피처·학습 구간·네트워크 설정을 순차 검증하고 있습니다.
...
학습용 코드는 개념 정리(`lesson.md`) → 직접 구현 → 단계별 검사(`check_XX.py`) 순서로 구성했습니다.
일부 파일의 `TODO` 주석은 미구현 표시가 아니라, 직접 구현했던 학습 지점을 보존한 기록입니다. 구현 여부는 각 단계의 `check_XX.py`와 CI로 확인합니다.
...
- 다종목 일반화와 보상 설계 확장은 후속 과제로 남겨 두었습니다.
